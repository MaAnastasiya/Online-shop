...
public class ProxyConnection implements Connection { // отвечает за открытие соединения с базой данных
    ...
    public Statement createStatement() throws ConnectionException {
        try {
            return connection.createStatement(); // экземпляр для sql запросов
        } catch (SQLException e) {
            throw new ConnectionException("Error in createStatement", e);
        }
    }

    // разрушаем соединение
    ...
    // На вход sql строка запроса к БД
    @Override
    public PreparedStatement prepareStatement(String sql) throws ConnectionException {
        try {
            return connection.prepareStatement(sql);// форматирование запроса
        } catch (SQLException e) {
    ...
    // Создает значение по умолчанию PreparedStatement объект,
    // у которого есть возможность получить автоматически сгенерированные ключи.
    @Override
    ...
    // Получает Connection текущее имя схемы объекта.
    @Override
    ...
    // аварийное прекращение работы соединения
    @Override
    ...
    // Устанавливает максимальный период Connection или объекты,
    // создаваемые из Connection будет ожидать базы данных
    @Override
    ...
    // возвращает число миллисекунд, которые драйвер будет ожидать запроса к базе данных
    @Override
